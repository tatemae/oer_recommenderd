...
public class SubjectAutoGenerator extends Base 
{
	...
	private void autoGenerateSubjects(HashSet<String> hsSubjects, int nEntryID, int nSubjects, IndexReader reader, IndexSearcher searcher)
	{
		try 
		{
			// find the document
	...
	private int addEntrySubject(HashSet<String> hsSubjects, int nEntryID, String sSubject) throws SQLException
	{
	...
	private void autoGenerateSubjects() throws Exception
	{
		...
		PreparedStatement pstGetEntrySubjects = cn.prepareStatement("SELECT subjects.name FROM entries_subjects INNER JOIN subjects ON entries_subjects.subject_id = subjects.id WHERE entries_subjects.entry_id = ? LIMIT 5");
		...
		pstAddEntrySubject = cn.prepareStatement("INSERT INTO entries_subjects (subject_id, entry_id, autogenerated) VALUES (?, ?, true)");
		nNewEntrySubjects = 0;
